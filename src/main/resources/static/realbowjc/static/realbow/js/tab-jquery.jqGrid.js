...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		setPager = function (pgid, tp){
			...
			$("#"+$.jgrid.jqID(pgid) )
			.append("<div id='"+pgcnt+"' class='ui-pager-control' role='group'><table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table' style='width:100%; table-layout:fixed; height:35px;' role='row'><tbody><tr>" +
					"<td id='"+cent+"' align='center' style='white-space:pre;'></td><td id='"+rgt+"' align='right'></td></tr></tbody></table></div>")
...
//				}
				str +="<input id=\"ui-pg-inputbox\" type='text' value=\""+ts.p.rowList[0]+"\" class='ui-pg-inputbox' style='width:35px; height:20px;' maxlength='7'></input> 行 / 页</td>";
			}
			if(dir==="rtl") { pgl += str; }
			...
			//输入每页显示总数
			$("#ui-pg-inputbox").keypress(function(e) {
				var key = e.charCode || e.keyCode || 0;
				if(key === 13) {
					if(!clearVals('records')) { return false; }
					ts.p.page = Math.round(ts.p.rowNum*(ts.p.page-1)/$(this).val()-0.5)+1;
					ts.p.rowNum = $(this).val();
					if(ts.p.pager) { $('.ui-pg-selbox',ts.p.pager).val($(this).val()); }
					if(ts.p.toppager) { $('.ui-pg-selbox',ts.p.toppager).val($(this).val()); }
					populate();
					return false;
				}
				return this;
...
$.jgrid.extend({
    ...
    columnChooser : function(opts) {
        ...
        $.each(colModel, function(i) {
        	if($.jgrid.stripHtml(colNames[i]) == '净化器ID' || $.jgrid.stripHtml(colNames[i]) == '风机ID'){
        		return;
        	}else{
	            colMap[this.name] = i;
	            if (this.hidedlg) {
